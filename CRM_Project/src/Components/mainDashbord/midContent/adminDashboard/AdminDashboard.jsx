...
const BOX_HEIGHT = Math.round(153 * 0.7); // 30% decrease
const BOX_WIDTH = 245;
const BOX_WIDTH_EXPANDED = Math.round(BOX_WIDTH * 1.2); // 20% increase
const BOX_HEIGHT_EXPANDED = Math.round(BOX_HEIGHT * 1.2); // 20% increase
...
const AdminDashboard = () => {
  ...
  // Removed barHover state (no hover effect)
  ...
  const gap = sidebarCollapsed ? 64 : 32; // Increase gap when collapsed
  // Calculate grid width so side gaps = gap between boxes
  // Use 100% width for grid when collapsed for equal side gaps
  const gridWidth = '100%';
